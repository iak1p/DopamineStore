    ...
    <main class="container mx-auto px-3 pb-12">
        <nav class="mt-6 text-sm text-[#8A8A8A]">
            <a href="{{ route('shop', ['category' => $product['category']]) }}"
                class="hover:underline">{{ ucfirst(implode(' ', explode('-', $product['category']))) }}</a>
        ...
        <section class="mt-8 grid grid-cols-1 gap-10 lg:grid-cols-2">
            <div class="flex gap-6">
                <div class="hidden flex-col gap-3 sm:flex">
                    <button class="h-16 w-16 rounded-xl border border-[#EAEAEA] bg-[#F6F6F6] p-2">
                        <img src="/phase1/templates/assets/img/peach.png"
                            class="h-full w-full object-contain" alt="Peach thumbnail" />
                    </button>
                    <button class="h-16 w-16 rounded-xl border border-[#EAEAEA] bg-[#F6F6F6] p-2">
                        <img src="/phase1/templates/assets/img/peach.png"
                            class="h-full w-full object-contain" alt="Peach thumbnail" />
                ...
                <div
                    ...
                    <img src="/phase1/templates/assets/img/peach.png" alt="Peach"
            ...
            <div>
                <h1 class="text-3xl font-semibold tracking-tight text-[#111] sm:text-4xl">
                    {{ $product['name'] }}
                    <span
                        class="ml-2 text-sm font-medium text-[#8A8A8A]">{{ $product['description'] }}</span>
                </h1>
                ...
                <div class="mt-6">
                    ...
                    <p class="mt-1 text-sm text-[#9A9A9A]">
                        Unit price: 3.90 € per 1 kg
                    </p>
                </div>

                <p class="mt-6 max-w-xl text-sm leading-7 text-[#4A4A4A]">
                    Sweet, juicy and carefully selected peaches with a
                    delicate aroma and soft texture. Ideal for everyday
                    snacking, desserts, smoothies or fruit salads.
                </p>
