<header class="container mx-auto py-3 px-3">
    <div class="sm:flex sm:justify-between sm:items-center flex-row gap-3">
        ...
        <form action="{{ route('shop') }}" method="GET" class="w-full relative">
            <input type="text" class="border border-[#D9D9D9] rounded-xl px-3 py-5 w-full text-sm"
                placeholder="Search for products" id="search" name="search" autocomplete="off"
                value="{{ request('search') }}" />

            <div id="searchSuggestions"
                class="absolute top-full left-0 w-full bg-white border border-[#D9D9D9] rounded-xl mt-2 shadow-lg hidden z-50">
            </div>
        </form>
            ...
            timeout = setTimeout(() => {
                fetch(`/search-suggestions?search=${encodeURIComponent(query)}`)
                    ...
                    .then(data => {
                        ...
                        <a href="/product/${product.slug}" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-100 border-b last:border-b-0">
                            <img
                                src="${product.image}"
                                alt="${product.alt}"
